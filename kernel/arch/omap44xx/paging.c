...
{
    printk(LOG_NOTE, "paging_arm_reset 0x%"PRIxLPADDR" to 0x%"PRIxLPADDR"\n",
           paddr, (paddr + bytes));
    ...
    // map high-mem relocated exception vector to corresponding section
    // core 0: 0xffff0000 -> 2G
    // core 1: 0xffff0000 -> 2G + 1MB
    // ...
    // FIXME: quite a lot of space is wasted!!  maybe we want to do something
    // about this
    int core_id = hal_get_cpu_id();
    lpaddr_t addr = PHYS_MEMORY_START + (core_id * LARGE_PAGE_SIZE);
