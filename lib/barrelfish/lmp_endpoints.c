...
errval_t lmp_endpoint_setup(size_t epoffset, size_t buflen, struct lmp_endpoint **retep)
{
    dispatcher_handle_t handle = disp_disable();
    uint8_t *epbuf = (uint8_t*)get_dispatcher_vaddr(handle) +
                        get_dispatcher_size() + epoffset;
    struct lmp_endpoint *ep = (struct lmp_endpoint *)epbuf;
    ...
    endpoint_init(ep);
    // add sentinel word that should be there if minting in the
    // kernel does the right thing
    ep->buflen = buflen + 1;
 ...
 */
// this is the EP that is created in the kernel
#define FIRSTEP_BUFLEN (sizeof(struct lmp_endpoint) + \
                        (DEFAULT_LMP_BUF_WORDS + 1) * sizeof(uintptr_t))
void lmp_endpoint_init(void)
{
    dispatcher_handle_t handle = disp_disable();
    size_t heapoffset = get_dispatcher_size() + FIRSTEP_BUFLEN;
    void *buf = (char *)get_dispatcher_vaddr(handle) + heapoffset;
    size_t buflen = (1UL << DISPATCHER_FRAME_BITS) - heapoffset;
