                              ...
                              struct spawn_state *st, struct bootinfo *bootinfo)
{
    ...
    while (remain > 0) {
        ...
        regions[*regions_index].mrmod_slot = *slot - 1;
        (*regions_index)++;
...
// for UART related information
extern lpaddr_t uart_base[NUM_PORTS];
extern size_t uart_size[NUM_PORTS];

struct dcb *spawn_module(struct spawn_state *st, struct cte *rootcn,
                         const char *name, int argc, const char** argv,
                         lpaddr_t bootinfo, lvaddr_t args_base,
                         alloc_phys_func alloc_phys, lvaddr_t *retparamaddr)
{
    printk(LOG_NOTE, "spawn_module\n");
    errval_t err;
    ...
    lpaddr_t modulecn_addr = alloc_phys(1UL << (OBJBITS_CTE + MODULECN_SIZE_BITS));
