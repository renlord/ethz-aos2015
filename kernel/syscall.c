...
#if 0
// FIXME: Put this prototype in proper header file
void boot_application_core(coreid_t core_id, lvaddr_t entry);
struct sysret sys_bootcore(coreid_t core_id, lvaddr_t entry)
{
    struct sysret ret;
    printk(LOG_NOTE, "booting another core\n");
    errval_t err = start_aps_arm_start(core_id, entry);
//    boot_application_core(core_id, entry);
    ret.error = err;
    return ret;
}
#endif // 0

struct sysret sys_getchar(void)
{

    char ch = serial_getchar(serial_console_port);
    //printf("input is %c\n", ch);
    struct sysret ret;
    ret.value = (uintptr_t) ch;
...
struct sysret sys_yield(capaddr_t target)
{
    ...
    if (yield_to != NULL) {
        ...
        struct dispatcher_shared_generic *listener_disp1 =
            get_dispatcher_shared_generic(target_dcb->disp);
        printf("sys_yield: from %.*s to %.*s\n",
             DISP_NAME_LEN, current_disp1->name,
             DISP_NAME_LEN, listener_disp1->name);
