...
uint8_t core_id = 0;  // core_id value so that all printk will have it
// This value should be set somewhere in early bootup (eg: arch_init).
...
void printk(int level, const char *msg, ...)
{
    if(kernel_loglevel > level) {
        ...
        printf("kernel[%d] %.*s", hal_get_cpu_id(), (int)sizeof(buf), buf);
    }
}
