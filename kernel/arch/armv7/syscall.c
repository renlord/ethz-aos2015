    ...
    )
{
    ...
    printf("dispatcher_dump_ptables\n");
    ...
    if (err_is_ok(r.error))
    {
        ...
        if (ObjType_EndPoint == to->type)
        {
            ...
            if (listener->disp) {
                ...
                debug(SUBSYS_SYSCALL, "LMP msg: from %.*s to %.*s\n",
        ...
        r.error = err_push(r.error, SYS_ERR_INVOCATION_TARGET_LOOKUP);
        debug(SUBSYS_SYSCALL, "caps_lookup_cap failed for invocation target\n");
    }
...
void sys_syscall(arch_registers_state_t* context)
{
    ...
    switch (syscall)
    {
        ...
        case SYSCALL_BOOT_CORE:
            if (argc == 3)
            {
                r = sys_monitor_spawn_core((coreid_t)sa->arg1,
                        CPU_ARM,
                        (lpaddr_t)sa->arg2);
               // r = sys_bootcore((coreid_t)sa->arg1, (lvaddr_t)sa->arg2);
                break;
            }
