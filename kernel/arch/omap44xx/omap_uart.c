...
static void omap_uart_init(omap_uart_t *uart, lvaddr_t base)
{
    ...
    ier = omap_uart_IER_modem_sts_it_insert(ier, 0);  // Modem status irq - no
    ier = omap_uart_IER_rhr_it_insert(ier, 1);        // Receive irq - yes
    omap_uart_IER_wr(uart, ier);

    // Enable FIFOs and select highest trigger levels
    // Enable FIFOs and select lowest trigger levels
    omap_uart_FCR_t fcr = omap_uart_FCR_default;
    fcr = omap_uart_FCR_rx_fifo_trig_insert(fcr, 0);
    fcr = omap_uart_FCR_tx_fifo_trig_insert(fcr, 0);
...
errval_t serial_init(unsigned port)
{
    ...
    printk(LOG_NOTE, "omap serial_init base = 0x%"PRIxLVADDR" 0x%"PRIxLVADDR"\n",
	   base, base + offset);
